...
AFTER DELETE ON ORDER_ITEM
    BEGIN
		...
		TAX_TOTAL= (
			SELECT 
			    coalesce(avg((B.TAX_PERCENTAGE * (B.PREPARATION_COST + ITEMS_TOTAL_PRICE) / 100)), 0)
            FROM ORDER_ITEM AS A
...
CREATE TRIGGER UpdateOrderFinalAmount_UPDATE
...
AFTER UPDATE ON ORDER_BASE
    BEGIN
        UPDATE CLIENT
			...
			WHERE
				CODE = new.CLIENT_CODE AND NEW.ORDER_STATUS LIKE 'COMPLETADA';
	END;